...
from tools.backend import call_node, call_nodes
from tools.registry_cache import get_autnum_field, has_autnum
...
def get_whoisinfo_by_asn(asn, item=...):
    ...
    registry_value = get_autnum_field(asn, item)
    if registry_value:
        return _normalize(registry_value)
...
def call_agent_action(action, data, region, *, timeout=10, retry=2, backoff_factor=0.1):
    return call_node(action, data, region, timeout=timeout, retry=retry, backoff_factor=backoff_factor)
