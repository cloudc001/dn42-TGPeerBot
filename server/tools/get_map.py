        ...
        for rank_type, rank_data in temp_data.items():
            ...
            for index, (asn, value) in enumerate(s, 1):
                if value != last_value:
                    rank_now = index
                last_value = value
                # Avoid blocking map refresh on thousands of WHOIS lookups.
                # Keep a fast placeholder so commands depending on map data return quickly.
                if asn not in as_name:
                    as_name[asn] = asn_desc.get(asn, str(asn))
