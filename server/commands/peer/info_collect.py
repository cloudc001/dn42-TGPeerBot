...
def post_confirm(message, peer_info):
    ...
    try:
        r = tools.call_agent_action("peer", json.dumps(peer_info), peer_info["Region"], timeout=10)
        if r.status == 503:
            bot.send_message(
                message.chat.id,
                (
        ...
        if progress_type == "modify":
            ...
            if old_region == peer_info["Region"]:
                r = tools.call_agent_action("restart", str(db[message.chat.id]), peer_info["Region"], timeout=10)
                if r.status != 200:
                    bot.send_message(
                        message.chat.id,
                        (
                ...
                r = tools.call_agent_action("remove", str(db[message.chat.id]), old_region, timeout=10)
                if r.status != 200:
                    bot.send_message(
                        message.chat.id,
                        (
