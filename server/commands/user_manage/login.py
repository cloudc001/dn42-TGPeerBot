...
def whois_query_lines(query, timeout=4, retries=2):
    for _ in range(retries):
        try:
            output = subprocess.check_output(
                shlex.split(f"whois -h {config.WHOIS_ADDRESS} {query}"),
                timeout=timeout,
...
def whois_server_unavailable(raw_asn):
    try:
        probe = str(raw_asn).strip()
        if probe.upper().startswith("AS"):
            probe = probe[2:]
        int(probe)
    except ValueError:
        probe = "4242420000"
    try:
        output = (
            subprocess.run(
                shlex.split(f"whois -h {config.WHOIS_ADDRESS} AS{probe}"),
    ...
    return any(marker in output for marker in failure_markers) or output == ""


def get_email(asn):
    registry_emails = tools.get_registry_emails_for_asn(asn)
    if registry_emails:
        return registry_emails

    whois_as = whois_query_lines(f"AS{asn}", timeout=4, retries=3)
    if not whois_as:
        return set()

    nic_handles = []
    emails = set()
    for line in whois_as:
        key, _, value = line.partition(":")
        key = key.strip().lower()
        value = value.strip()
        if key in ("admin-c", "tech-c") and value:
            nic_handles.append(value)
        if key in ("e-mail", "contact") and EMAIL_PATTERN.fullmatch(value):
            emails.add(value)

    for handle in nic_handles:
        whois_contact = whois_query_lines(handle, timeout=6, retries=3)
        for line in whois_contact:
            key, _, value = line.partition(":")
            key = key.strip().lower()
            value = value.strip()
            if key in ("e-mail", "contact") and EMAIL_PATTERN.fullmatch(value):
...
def login_input_asn(exist_asn, message):
    ...
    if asn := tools.extract_asn(raw):
        ...
        markup.row_width = 1
        if emails:
            markup.add(*(KeyboardButton(email) for email in sorted(emails)))
    ...
    elif whois_server_unavailable(raw):
        bot.send_message(
            message.chat.id,
            (
                f"Whois server is unavailable, please contact {config.CONTACT}\n"
                f"Whois 服务器故障，请联系 {config.CONTACT}"
            ),
            parse_mode="Markdown",
            reply_markup=ReplyKeyboardRemove(),
        )
